...
class Farming(Actions):
    ...
    def collect_commenters(self, profile_url):
        ...
        load_more = WebDriverWait(self.browser, 5).until(EC.presence_of_element_located((By.XPATH, "//span[contains(@aria-label, 'Load more comments')]")))
    ...
    def farm_likers(self):
        ...
        while a/(b-c) != 1.0:
            ...
            for i in range(1, num):
                try:
                    tab_1 = self.browser.current_window_handle
                    ...
                    if self.is_spam_prevention() == True:
                        ...
                        for usr in self.private_list:
                            if usr.url == self.browser.current_url:
                                ...
                                self.browser.close()
                                self.browser(switch_to.window(tab_1))
...
def like_posts(self, number_of_likes, number_of_posts, profile_url):
    first_post = self.browser.find_elements_by_xpath("//div[contains(@class, 'Nnq7C weEfm')][1]/div[1]")
    first_post.click()
    ...
    for x in range(1, like_count):
        self.like_picture()
        try:
            next_post = self.browser.find_elements_by_xpath("//a[contains(@class, 'coreSpriteRightPaginationArrow')]")
            next_post.click()
        except NoSuchElementException:
            pass
